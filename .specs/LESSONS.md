...
- **2026-06-13** | `fix-autoclip-encoder-batching` — **异步竞态**：`addMedia` 返回后才注册 `task-end` 监听获取 AID，两次导出几乎同时完成时第二次在第一次注册监听前检查 Map → 看不到 AID → 重复创建投稿。教训：在 `await` 之前写入 pending Promise 占位，用 Promise 而非裸值做并发安全的 Map 缓存。触发条件：任何"检查 Map → await → 写入 Map"的模式都是竞态。（`{{title}}`/`{{user}}`/`{{roomId}}`）。触发条件：下次新增公共模板变量时统一提取 `TemplateVariableProvider` 接口。
