...
- **2026-06-11** | `fix-autoclip-three-bugs` — R3: autoclip 的 TemplateContext 与录制系统的 webhook formatTitle 各自维护模板变量定义（`{{title}}`/`{{user}}`/`{{roomId}}`）。触发条件：下次新增公共模板变量时统一提取 `TemplateVariableProvider` 接口。
