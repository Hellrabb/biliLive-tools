...
export async function runAutoClipPipeline(
  ...
  if (filterConfig.enabled && stats.danmu.length > 0) {
    ...
    let filterResult: { filtered: typeof stats.danmu; removed: number; breakdown: Array<{ ruleId: string; pattern: string; removed: number }> } | undefined;
  ...
  return {
    id, videoPath, danmuPath, highlights, llmFallback, suspiciousPatterns, danmakuFilterStats,
    autoGeneratedRules: autoGeneratedRules.length > 0 ? autoGeneratedRules : undefined,
  };
}
