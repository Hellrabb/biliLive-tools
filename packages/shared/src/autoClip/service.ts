...
export class AutoClipService {
  ...
  async analyzeAndSave(params: {
    ...
    // Rules are collected in the pipeline without mutating presetConfig
    // to avoid race conditions with concurrent analyses.
    ...
    if (activePresetId && result.autoGeneratedRules && result.autoGeneratedRules.length > 0) {
      try {
        ...
        if (preset) {
          ...
          const newRules = result.autoGeneratedRules.filter((r) => !existingPatterns.has(r.pattern));
